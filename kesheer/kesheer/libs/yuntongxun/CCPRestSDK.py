...
class REST:
    ...
    def CreateSubAccount(self, friendlyName):
        ...
        # xml格式
        body ='''
            <?xml version="1.0" encoding="utf-8"?><SubAccount><appId>%s</appId>\
    ...
    def landingCall(self, to, mediaName, mediaTxt, displayNum, playTimes, respUrl, userData, maxCallTime,
                    speed, volume, pitch, bgsound):
        ...
        body ='''<?xml version="1.0" encoding="utf-8"?><LandingCall>\
            ...
            ''' % (to, mediaName, mediaTxt, self.AppId, displayNum, playTimes, respUrl, userData, maxCallTime,
                   speed, volume, pitch, bgsound)
        if self.BodyType == 'json':   
            ...
            "bgsound": "%s"}''' % (to, mediaName, mediaTxt, self.AppId, displayNum, playTimes, respUrl, userData,
    ...
    def voiceVerify(self, verifyCode, playTimes, to, displayNum, respUrl, lang, userData):
        ...
        if self.BodyType == 'json':   
            ...
            "lang": "%s","userData": "%s"}''' % (self.AppId, verifyCode, playTimes, to, respUrl, displayNum, lang, userData)
        req.add_data(body)
        data = ''
        try:
            res = urllib2.urlopen(req)
    ...
    def CallResult(self, callSid):
        ...
        # 拼接URL
        url = "https://"+self.ServerIP + ":" + self.ServerPort + "/" + self.SoftVersion + "/Accounts/" + self.AccountSid\
              + "/CallResult?sig=" + sig + "&callsid=" + callSid
        # 生成auth
        src = self.AccountSid + ":" + self.Batch
        auth = base64.encodestring(src).strip()
        req = urllib2.Request(url)
    ...
    def QueryCallState (self, callid, action):
        ...
        # 拼接URL
        url = "https://"+self.ServerIP + ":" + self.ServerPort + "/" + self.SoftVersion + "/Accounts/" + self.AccountSid\
              + "/ivr/call?sig=" + sig + "&callid=" + callid
        # 生成auth
        src = self.AccountSid + ":" + self.Batch
        auth = base64.encodestring(src).strip()
        req = urllib2.Request(url)
    ...
    def MediaFileUpload (self, filename, body):
        ...
        # 拼接URL
        url = "https://"+self.ServerIP + ":" + self.ServerPort + "/" + self.SoftVersion + "/Accounts/" + self.AccountSid\
              + "/Calls/MediaFileUpload?sig=" + sig + "&appid=" + self.AppId + "&filename=" + filename
        # 生成auth
        src = self.AccountSid + ":" + self.Batch
        auth = base64.encodestring(src).strip()
        req = urllib2.Request(url)
